...
> 源：`freemarker-core/src/main/java/freemarker/core/BuiltInsFor*.java`（30 文件，**133 个 BI 静态类**）
...
| `c` / `cn` | canonical 数字字符串（CFormat 输出，见 08 文档） |
| `date` / `number`（多类型版） | 类型转换（数字→日期、日期→数字？**对照**：`?number` 在字符串上；`?date` 在数字上为 epoch） |
| `size` | 序列/集合/哈希大小（HashEx） |
| `is_*` 全家族（18 个） | `is_boolean/is_collection/is_collection_ex/is_directive/is_enumerable/is_hash/is_hash_ex/is_indexable/is_macro/is_markup_output/is_method/is_node/is_number/is_sequence/is_string/is_transform` + `is_infinite/is_nan`（数字）—— 角色判定（映射 TModel 槽位检查，见 02 文档 §4.1） |
...
4. **`?c` 数字格式**：`?c` 输出与 locale 无关 canonical 形式（整数无小数点、大数无指数？）—— 由 CFormat 统一（08 文档）。
...
1. 133 个 BI 全部注册且可通过名称查表（编译期枚举核对清单）。
