...
- Rust 形态：`freemarker/tests/suite/` 下目录结构：
  ```
  tests/suite/
  ├── cases/            # 每用例一个目录（name.ftl + name.data.json + name.expected.txt）
  ├── manifest.rs       # include_str! 嵌入清单（编译期数据，决策 D2）
  └── runner.rs         # 通用 runner：parse → 数据注入（JSON→TModel）→ 渲染 → diff
  ```
...
- 错误用例（`errorExpected` 属性）：断言 `TemplateError` 消息与 `expected` 一致（09 文档 §4 容忍清单内允许差异）。
...
- **错误消息对比**：`scripts/compare_errors.py` 对照 09 文档 §4 基线。
