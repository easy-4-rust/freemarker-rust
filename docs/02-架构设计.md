...
| 领域 | crate | 版本策略 | 说明 |
|---|---|---|---|
| 正则 | `regex` | 1.x | 对齐 Java `java.util.regex` 语义（注意差异：见 05 文档正则节） |
...
| Python | `pyo3` 0.29（仅 freemarker-pyo3） | 锁定 | 见 10 文档 |
...
消息文本由 `error/messages.rs` 统一生成，逐字对齐 Java（见 09 文档）。
