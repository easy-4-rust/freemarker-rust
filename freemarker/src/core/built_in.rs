...
// 辅助族按 #[path] 聚合拆分（参照 grammar.rs 模式）：
// - built_in_helpers.rs —— 循环状态、new 构造器、?interpret、参数解析辅助
#[path = "built_in_helpers.rs"]
mod built_in_helpers;

pub(crate) use self::built_in_helpers::check_legacy_escaping_ban;
use self::built_in_helpers::{
    arg_expr, builtin_interpret, char_index_from, date_type_if_unknown, eval_item_cycle_direct,
    is_type_test, locale_case, loop_state_builtin, parse_number, str_builtin,
