//! 逻辑或表达式 —— 对应 Java `freemarker.core.OrExpression`
...
/// 逻辑或表达式（对应 OrExpression.java；解析器经 `ExprKind::Or` 承载）
...
impl OrExpression {
    ...
    pub(crate) fn eval(&self, env: &mut crate::core::Environment) -> Result<TModel> {
        eval_or(env, &self.left, &self.right)
    }
}

/// 短路或（Java OrExpression：lho.evalToBoolean || rho.evalToBoolean）
pub(crate) fn eval_or(env: &mut crate::core::Environment, a: &Expr, b: &Expr) -> Result<TModel> {
    let lm = eval(env, a)?;
